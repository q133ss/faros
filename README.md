...
## Установка
```bash
docker exec -it app_faros composer install
```
...
- **Запуск миграций:**
  ```bash
  docker exec -it app_faros php artisan migrate
  ```

- **Очистка кэша конфигурации:**
  ```bash
  docker exec -it app_faros php artisan config:cache
  ```

- **Создание новой миграции:**
  ```bash
  docker exec -it app_faros php artisan make:migration create_users_table
  ```

- **Заполнение базы данных данными:**
  ```bash
  docker exec -it app_faros php artisan db:seed
  ```
...
```bash
alias art="docker exec -it app_faros php artisan"
```
