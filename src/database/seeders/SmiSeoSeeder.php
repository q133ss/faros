...
class SmiSeoSeeder extends Seeder
{
    /**
    ...
    {
        $seoList = $this->readCsv('database/seeders/data/smi_seo_data.csv');
        foreach ($seoList as $case) {
            ...
            $slug = str_replace('/', '', str_replace('https://faros.media/smi', '', $case['URL']));
            $smi = Smi::where('slug', $slug)->first();

            $data = [
                'seable_type' => Smi::class,
                'seable_id' => $smi->id,
